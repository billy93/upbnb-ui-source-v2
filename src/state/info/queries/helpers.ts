...
import { TransactionType } from '../../info/types'
...
export const fetchChartData = async (
  ...
  while (!allFound) {
    ...
    const { data, error: fetchError } = await getEntityDayDatas(skip, address ? address : "")
    skip += 1000
    allFound = data != null ? data.length < 1000 : false
    error = fetchError
    if (data) {
